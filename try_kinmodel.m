...
T = p_new; % trenutna pozicija arm_left_7_link

% T=[0.19;0.3047;-0.0119]; % trenutna pozicija arm_left_7_link
Kp=1;
tsamp=0.01;
dt=2;
...
e=1;
while norm(e) > 0.02
    msg_states = receive(sub_states, 1);
    q=msg_states.Position(1:7);
%     [x,R,J]=kinmodel_talos_left_arm(q);
    [x,R,J]=kinmodel_talos_arm(q);
    e = T-x;
    J = J(1:3,:);
    qd=pinv(J)*Kp*e;
    novi_q= novi_q+qd*tsamp;
    move(LLeg,RLeg,novi_q,RArm,Head,Torso,0.1);
    disp("Here")
    disp(norm(e))
%     pause(0.01)
end
